	...
	**/
	...
	function get_date_content( $date )
	{
		...
		else 
		{
			if( is_numeric($date) )
			{
				$str = date('Y',$date).'年'.date('m',$date).'月'.date('d',$date).'日';
			}
			else
			{
				$str = date('Y',strtotime($date)).'年'.date('m',strtotime($date)).'月'.date('d',strtotime($date)).'日';
			}
		}
...
//	print_r($CI->db->last_query());exit;
	...
	***/
	...
	function get_stay_china_year($num='1')
	{
		...
		{
			switch($num){
				case '0':return "半年及以下";break;
				case '1':return "1年";break;
				case '2':return "2年";break;
				case '3':return "3年";break;
				case '4':return "4年以及以上";break;
				default:return "未知";
			}
		}
