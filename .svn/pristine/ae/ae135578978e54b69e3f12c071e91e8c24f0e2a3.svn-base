...
//				$(this).parent().find('input').val(oldval);
			...
			$(".save-info").click(function(){
				...
				$.ajax({
					url: api_url, 
					data: {action:'update_userinfo',id:id,school:realname}, 
					type :'POST',
...
/*				$.post("/UserAjax/updateUserInfo.html",{ realname:realname },function(res){
    ...
    var signature = {
        ...
        bind: function (){
        	...
        	$('.save-info').click(function(){
        		var signature = $('input:radio[name="sex"]:checked').val();
        		console.log(signature)
        		if(signature == ''){
        			notice('您的性别不能为空');
        			return false;
        		}
        		// signature=signature=='男'?1:2;
				var api_url = $('#api_url').val()+'/api/update_userinfo';
				var id = $('#user_id').val();
				$.ajax({
					url: api_url, 
					data: {action:'update_userinfo',id:id,sex:signature}, 
					type :'POST',
					success: function( data )
					{		
						data = eval('(' + data + ')');	
...
/*        		$.post("/UserAjax/updateUserInfo.html",{ signature:signature },function(res){
   ...
   // grade
    var grade2 = {
        ...
        bind: function (){
        	$('#grade').val($('#hgrade').val());
        	$('.save-info').click(function(){
        		var signature = $('#changeGrade').val();
        		// console.log(signature)
        		if(signature == ''){
        			notice('您的性别不能为空');
        			return false;
				...
				$.ajax({
					url: api_url, 
					data: {action:'update_userinfo',id:id,grade:signature}, 
					type :'POST',
					success: function( data )
					{		
						data = eval('(' + data + ')');	
						if(data.status){
							...
							console.log(signature)
							// signature=signature==1?'男':'女';
							$("#hgrade").val(signature);
							// checkcomplete();
							// console.log($("#hgrade").val())
							history.go(-1);														
						}else{
...
function change() {
 ...
 function html5Reader(file){
     ...
     reader.onload = function(e){
		...
		var postdata = 'action=upload_image&type=icon&image=';
//console.log(postdata);		
		...
		var clipArea = new bjj.PhotoClip("#clipArea", {
			...
			clipFinish: function (dataURL) {
				...
				$.post(api_url,{ image:dataURL,action:'upload_image',type:'icon','id':id },
					function(data)
					{
						data = eval('(' + data + ')'); 
						if(data.status == 1){
...
/*				$.post(api_url,data:postdata,function(data){
					if(data.status){
						//成功
						setTimeout(function(){
							tusiload('false');
