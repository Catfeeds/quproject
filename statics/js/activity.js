...
function requestdata(type,businessType,statusType,sortType,page,first,top){
	...
	// $.ajax({
	// 	 type: "POST",
	// 	 url: "/ActivityAjax/getactivity.html",
	// 	 data: { businessType:businessType,statusType:statusType,sortType:sortType,city:city,page:page,length:10,first:first},
	// 	 dataType: "json",
	// 	 success: function(data){
	// 		if(data.status == 1){
	// 			for(var i=0;i<data.list.length;i++){
	// 				html+='<li>';
	// 				html+='  <a href="/u/'+ data.list[i].uguid +'" class="to-detail">';
	// 				html+='		<div class="publisher"><img src="'+ data.list[i].headimgurl +'"><span>'+ data.list[i].realname +'</span></div>';
	// 				html+='  </a>';

	// 				html+='  <a href="/p/'+data.list[i]['guid']+'" class="to-detail">';
	// 				html+='	<div class="ad_pic error">';
	// 				html+='		<img class="lazy" src="/Public/Home/images/error_img.png" data-original="'+data.list[i].bigcover+'" alt="">';
	// 				html+='		<span class="'+ data.list[i]['class'] +' clear"><i></i>'+ data.list[i]['activitystatus'] +'</span>';
	// 				html+='	</div></a>';
					
	// 				html+='  <a href="/p/'+data.list[i]['guid']+'" class="to-detail">';
	// 				html+='		<div class="title"><p>'+data.list[i].proname+'</p></div>';
	// 				html+='		<div class="ad_info clear">';
	// 				html+='			<span>活动时间：'+data.list[i].begintime+'</span>';
	// 				html+='			<span>活动地点：'+data.list[i].city+'</span>';
	// 				html+='			<span>募集人数：'+data.list[i].raise_num+'/'+data.list[i].demand_num+'人</span>';

	// 				if(data.list[i].unit_price == 0){
	// 					html+='			<span>酬谢资金：面议</span>';
	// 				}else{
	// 					html+='			<span>酬谢资金：'+data.list[i].unit_price+'元/人</span>';
	// 				}	
