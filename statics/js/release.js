     ...
     reader.onload = function(e){
			...
			var api_url = $('#api_url').val()+'/api/upload_img';
			  if(operate != 'insert_pic'){
				  //上传图片到服务器 picurl图片路径
				  tusiload('上传中');
				  $.post(api_url,{ image:dataURL,action:'upload_image',type:'event',width:320,height:320},
				  	function(res)
				  	{
				  		res = eval('(' + res + ')'); 
						tusiload('false');
						if(res.status == 1){
							tusi(res.msg);
							if(res.image != ''){
								$('#file_box').css('background-image','url('+res.image+')');
								$('#cover').val(res.image);
								$('.pic-inner').show();
								$('.uploadbtn').hide();
							}
				  ...
				  //上传图片到服务器 picurl图片路径
				  tusiload('上传中');
				  $.post(api_url,{ image:dataURL,action:'upload_image',type:'event',width:640,height:9999 },
				  	function(res)
				  	{
				  		res = eval('(' + res + ')'); 
						tusiload('false');
